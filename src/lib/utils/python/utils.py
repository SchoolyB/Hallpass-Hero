        ...
        try:
            with open(statsLogFilePath, "r") as file:
                for line in file:
                    ...
                    elif keyName == "CurrentSessionStudentsProcessed":
                        bulk_data_loader.Statistic.CurrentSessionStudentsProcessed = int(value)
                        return bulk_data_loader.Statistic.CurrentSessionStudentsProcessed
...
def transfer_session_time_to_previous():
    __utils_runtime_logger("attempted to transfer session time to previous", "transfer_session_time_to_previous")
    try:
        with open(statsLogFilePath, "r") as file:
            lines = file.readlines()
        for i, line in enumerate(lines):
...
#                                  - CurrentSessionStudentsProcessed
...
def check_and_populate_stats_file():
    ...
    if(fileExists == True):
        ...
        try:
            with open(statsLogFilePath, "r") as file:
                content = file.read()
                if ("TotalStudentsProcessed" in content and "TotalSessionsCompleted" in content and "CurrentSessionNumber" in content and "CurrentSessionStudentsProcessed" in content and "CurrentSessionStartTime" in content and "CurrentSessionEndTime" in content and "PreviousSessionStartTime" in content and "PreviousSessionEndTime" in content):
                    return True
                else:
                    try:
                        with open(statsLogFilePath, "w") as file:
                            ...
                            file.write("CurrentSessionStudentsProcessed=0\n")
